...
const addBlockNameToModifiers = (array, blockname) => array.map((modifier) => `${blockname}_${modifier}`);

const generateModifiers = (array, blockname) => addBlockNameToModifiers(getModifiers(array), blockname).join(' ');

const getAffix = (name, blockname) => (name.trim() ? `${blockname}__${name}` : blockname);
